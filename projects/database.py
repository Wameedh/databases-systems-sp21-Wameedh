...
class DB:
        ...
        try:
            ...
            with connection.cursor() as cursor:
                if action == self.UPDATE:
                    sqlquery = query
                    if self.PROGRAME_STARTED:
                        sqlquery = self.record_trans(query, values)
                    cursor.execute(sqlquery)
                elif values:
                    if self.PROGRAME_STARTED:
                        self.record_trans(query, values)
                    cursor.execute(query, values) # query will have wildcards as values
                else:
                    if self.PROGRAME_STARTED:
                        self.record_trans(query)
    ...
    def run_sql_file(self, sqlfile):
        ...
        try:
            ...
            # After
            queries += self.get_queries_from("triggers.sql", "$$")
            queries_executed = 0
    ...
    def get_table_names(self):
        ...
        tables = self.select(query, value)
        return tables
    ...
    def record_trans(self, transaction, values=None):
        st1 = transaction
        st2 = transaction
        if values:
            index = 0
            if type(values) is dict:
                st1 = transaction % values
            else:
                for i in range(len(values)):
                    st1 = st2.replace("%s", str(values[index]), 1)
                    st2 = st1
                    index += 1
        f = open("transaction.sql", "a")
        f.write("\n" + st1)
        f.close()
        return st1